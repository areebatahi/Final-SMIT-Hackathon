...
const DonePage = () => {
    ...
    useEffect(() => {
        const fetchTasks = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    // Ensuring only tasks with 'Completed' status are set in state
    ...
    const handleUndoCompletion = async (taskId) => {
        ...
        try {
            ...
            const updatedStatus = task.status === 'Completed' ? 'In Progress' : 'Completed';  // Toggle status
            const response = await fetch(`${apiUrl}/tasks/${taskId}/status`, {  // Assuming new endpoint
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
                    ...
                    >
                        <div className="flex flex-col items-start space-y-1">
                            <div className="flex items-center space-x-2">
                                <button
                                    onClick={() => handleUndoCompletion(task._id)} // Update task status
                        ...
                        <button
                            onClick={() => handleDeleteTask(task._id)} // Use _id for deletion
